...
public class MissionService {
    ...
    @Transactional(readOnly = true)
    public MissionResDTO.ProceedingMissionPage getProceedingMissions(MissionReqDTO.ProceedingMissionRequest request) {
        ...
        Page<MemberMission> memberMissionPage = memberMissionRepository
                .findAllByMemberIdAndStatus(request.memberId(), MissionStatus.PROCEEDING, pageable);

        List<MissionResDTO.UserMissionItem> items = memberMissionPage.getContent().stream()
                .map(MissionConverter::toUserMissionItem)
                .toList();

        return new MissionResDTO.ProceedingMissionPage(
                items,
